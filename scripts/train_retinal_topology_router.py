...
from ilm.visual_lm.retinal_topology_router import (
    ...
    summarize_retinal_topology_router_trace,
)
...
ARCHITECTURE = "retinal-topology-router-v1"
PARTITION_SALT = "retinal-topology-router-v20"
EXPECTED_PVF_SHA256 = (
    "90791001203640f0de66316cf2e30b3e2c588480fef0e3d9d4f6283ba043ecbe"
)
PROTOCOL_DOCUMENT = "references/retinal_topology_router_v20_protocol.md"
CANDIDATE_SELECTION_RULE = {
    ...
    "requirements": [
        ...
        "correct_pixel_f1_dense > 0.68",
        "correct_pixel_f1_dense - field_shuffled_pixel_f1_dense > 0.12",
        "correct_pixel_f1_dense - zero_field_pixel_f1_dense > 0.10",
        "correct_identity_top1 > 0.70",
        ...
        "field_condition_pixel_l1 > 0.08",
        "zero_field_condition_pixel_l1 > 0.08",
        "occlusion_pixel_change > 0.02",
        "occlusion_locality > 0.40",
        "semantic_target_pixel_l1 > 0.05",
        "coarse_within_block_max < 1e-6",
        "detail_block_mean_abs_max < 1e-6",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train one preregistered V20 retinal topology-router arm from "
    ...
    parser.add_argument("--out", default="artifacts/retinal_topology_router_v20")
    ...
    parser.add_argument("--coarse-hidden-dim", type=int, default=256)
    ...
    parser.add_argument("--coarse-weight", type=float, default=0.25)
...
def _require_fixed_arguments(args: argparse.Namespace) -> None:
    if args.partition_salt != PARTITION_SALT:
        raise ValueError(f"V20 requires partition salt {PARTITION_SALT!r}")
    for group in (
        FIXED_MODEL_ARGUMENTS,
        FIXED_LOSS_ARGUMENTS,
        FIXED_OPTIMIZATION_ARGUMENTS,
        ...
        for name, expected in group.items():
            if getattr(args, name) != expected:
                raise ValueError(f"V20 requires --{name.replace('_', '-')}={expected}")
    if args.smoke:
        if args.maximum_steps > 20:
            raise ValueError("V20 smoke mode is limited to 20 optimization steps")
        return
    for name, expected in FIXED_EVIDENCE_ARGUMENTS.items():
        if getattr(args, name) != expected:
            raise ValueError(f"V20 evidence requires --{name.replace('_', '-')}={expected}")


def selection_rule(route_mode: str) -> dict[str, Any]:
    if route_mode == FIELD_ROUTE:
        return CANDIDATE_SELECTION_RULE
    if route_mode == GLOBAL_CONTROL_ROUTE:
...
def selection_gate_report(
    ...
    report = {
        ...
        "dense_pixel_f1": dense > 0.68,
        "identity_top1": (
            metrics["correct_identity_top1"] > 0.70
        ...
        "coarse_invariant": metrics["coarse_within_block_max"] < 1e-6,
        "detail_invariant": metrics["detail_block_mean_abs_max"] < 1e-6,
        "frozen_bank_sealed": (
        ...
        report.update(
            {
                "dense_field_shuffle_margin": (
                    dense - metrics["field_shuffled_pixel_f1_dense"] > 0.12
                ),
                "dense_zero_field_margin": (
                    dense - metrics["zero_field_pixel_f1_dense"] > 0.10
                ),
                "field_condition_pixel_l1": (
                    metrics["field_condition_pixel_l1"] > 0.08
                ),
                "zero_field_condition_pixel_l1": (
                    metrics["zero_field_condition_pixel_l1"] > 0.08
                ),
                "occlusion_pixel_change": metrics["occlusion_pixel_change"] > 0.02,
                "occlusion_locality": metrics["occlusion_locality"] > 0.40,
            }
        )
    return report
...
def paired_gate_report(
    ...
    *,
    ...
    return {
        ...
        "candidate_overall_noninferiority": (
            candidate["correct_pixel_f1"]
            >= control["correct_pixel_f1"] - 0.02
        ),
...
def validate_resume_checkpoint(
    ...
    *,
    ...
    expected_config: RetinalTopologyRouterConfig,
    smoke: bool,
) -> None:
    if checkpoint.get("architecture") != ARCHITECTURE:
        raise ValueError("resume is not a V20 topology-router checkpoint")
    if checkpoint.get("partition") != expected_partition:
        raise ValueError("resume uses a different V20 partition")
    ...
    restored = retinal_topology_router_config_from_payload(
        checkpoint["planner_config"]
    )
    if restored != expected_config:
        raise ValueError("resume uses a different topology-router configuration")
    ...
    if checkpoint.get("route_mode") != expected_config.route_mode:
        raise ValueError("resume uses a different V20 route mode")
    ...
    *,
    ...
    for batch_index, batch in enumerate(loader):
        ...
        with autocast_context(device, args.precision):
            loss, training_metrics, _ = retinal_topology_router_loss(
                planner,
                pvf.retina,
                target,
                semantic,
            ...
            _, trace = evaluate_retinal_topology_router_batch(
                planner,
                pvf.retina,
                target,
                semantic,
    ...
    if examples == 0:
        raise ValueError("development loader yielded no V20 examples")
    aggregate_trace = {key: torch.cat(values) for key, values in traces.items()}
    causal_metrics = summarize_retinal_topology_router_trace(
        aggregate_trace,
    ...
    report["selection_eligible"] = float(
        selection_eligible(report, planner.config.route_mode)
    )
    return report
...
def checkpoint_payload(
    ...
    *,
    ...
    return {
        ...
        "route_mode": planner.config.route_mode,
        "planner_config": retinal_topology_router_config_payload(planner.config),
        ...
        "arguments": vars(args),
        "selection_rule": selection_rule(planner.config.route_mode),
        ...
        "student_contract": {
            ...
            "output": "continuous spatial ink logits",
            "route_mode": planner.config.route_mode,
            "global_state_enters_detail_path": (
                planner.config.route_mode == GLOBAL_CONTROL_ROUTE
            ),
...
def _write_selection_receipt(
    ...
    reason = (
        "matched control and blinded readability gates remain"
        if route_mode == FIELD_ROUTE
        else "control checkpoint is evaluator-only and cannot open frozen data"
    )
        ...
        json.dumps(
            {
                ...
                "selected": selected,
                "arm_automatic_development_gate_passed": True,
                "paired_control_gate_passed": False,
                "blinded_readability_gate_passed": False,
...
def main() -> None:
    ...
    if not args.smoke and device.type != "cuda":
        raise ValueError("V20 evidence runs require one CUDA GPU")
    if device.type == "cuda":
    ...
    if pvf_sha256 != EXPECTED_PVF_SHA256:
        raise ValueError("V20 requires the preregistered V16 PVF checkpoint")
    ...
    protocol = {
        "architecture": ARCHITECTURE,
        "experiment": "V20 retinal topology router",
        ...
        "paired_selection_rule": {
            "candidate_dense_gain": "> 0.03",
            "candidate_overall_noninferiority": ">= control - 0.02",
    ...
    if protocol_path.exists():
        ...
        if existing != protocol:
            raise ValueError("existing preregistered V20 protocol differs")
    else:
    ...
    planner = RetinalTopologyRouter(config).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            with autocast_context(device, args.precision):
                loss, metrics, _ = retinal_topology_router_loss(
                    planner,
                    pvf.retina,
                    target,
                    semantic,
