...
from ilm.visual_lm.scaled_retinal_glyph_language_v46 import (
    ...
    scaled_retinal_glyph_language_v46_config_payload,
)
...
DEFAULT_FIELD = "artifacts/noise_limited_retinal_field_v45_20260814/field.pt"
DEFAULT_OUTPUT = "artifacts/scaled_retinal_glyph_language_v46_20260815"
SOURCE_FILES = (
    "ilm/visual_lm/scaled_retinal_glyph_language_v46.py",
    "ilm/visual_lm/scaled_retinal_glyph_language_v46_training.py",
    "ilm/visual_lm/scaled_retinal_glyph_language_v46_evaluation.py",
    "scripts/train_scaled_retinal_glyph_language_v46.py",
    "scripts/eval_scaled_retinal_glyph_language_v46.py",
)
FIXED_OPTIMIZATION = {
    "steps": 10_000,
    "batch_size": 8,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the frozen V46 scaled-retinal Chinese language core."
    )
...
def _data_boundary_receipt() -> dict[str, Any]:
    receipt = canonical_glyph_data_boundary_receipt()
    receipt["architecture"] = V46_ARCHITECTURE
    return receipt


def load_verified_v45_field(path: str) -> tuple[Any, dict[str, Any]]:
    checkpoint_sha256 = file_sha256(path)
    if checkpoint_sha256 != V46_REQUIRED_V45_CHECKPOINT_SHA256:
        raise ValueError("V46 V45 checkpoint digest differs from the protocol")
    payload = torch.load(path, map_location="cpu", weights_only=False)
    field = noise_limited_retinal_field_v45_from_checkpoint_payload(payload)
    field_state_sha256 = noise_limited_retinal_field_v45_state_sha256(field)
    if field_state_sha256 != V46_REQUIRED_V45_FIELD_STATE_SHA256:
        raise ValueError("V46 V45 field-state digest differs from the protocol")
    fit_sequence = payload.get("fit_sequence")
    if not isinstance(fit_sequence, Mapping) or (
        fit_sequence.get("joint_sha256")
        != V46_REQUIRED_V45_FIT_SEQUENCE_SHA256
    ):
        raise ValueError("V46 V45 fit sequence differs from the protocol")
    receipt = {
        "path": str(path),
        "checkpoint_sha256": checkpoint_sha256,
        "field_state_sha256": field_state_sha256,
        "fit_sequence_sha256": fit_sequence["joint_sha256"],
...
def _protocol_receipt(arguments: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "document": V46_PROTOCOL,
        "sha256": file_sha256(V46_PROTOCOL),
...
def _checkpoint(
    ...
    *,
    ...
    retinal_field_receipt: Mapping[str, Any],
    elapsed_seconds: float,
    peak_vram_gib: float,
    metrics: Mapping[str, float],
) -> dict[str, Any]:
    return {
        "experiment": V46_ARCHITECTURE,
        "architecture": V46_ARCHITECTURE,
        "model_config": scaled_retinal_glyph_language_v46_config_payload(
            model.config
        ),
        ...
        "retinal_field": dict(retinal_field_receipt),
        "data_boundary": _data_boundary_receipt(),
        "model_boundary": scaled_retinal_glyph_language_v46_boundary_receipt(model),
        "protocol": _protocol_receipt(arguments),
        "loss_weights": V46_LOSS_WEIGHTS.__dict__,
...
def main() -> None:
    ...
    if arguments["steps"] < 1 or arguments["batch_size"] < 1:
        raise ValueError("V46 training sizes must be positive")
    if arguments["gradient_accumulation"] < 1 or arguments["energy_samples"] < 2:
        raise ValueError("V46 accumulation and energy sampling are invalid")
    ...
    retinal_field, retinal_field_receipt = load_verified_v45_field(
        arguments["field"]
    )
    ...
    if (
        ...
        raise ValueError("V46 production requires the frozen corpus manifest")
    ...
    model_config = ScaledRetinalGlyphLanguageV46Config()
    start_update = 0
    ...
    if arguments["resume"]:
        ...
        if resume_payload.get("architecture") != V46_ARCHITECTURE:
            raise ValueError("resume checkpoint is not V46")
        if resume_payload.get("retinal_field") != retinal_field_receipt:
            raise ValueError("resume checkpoint has a different V45 field receipt")
        model_config = scaled_retinal_glyph_language_v46_config_from_payload(
    ...
    if start_update >= arguments["steps"]:
        raise ValueError("resume checkpoint already reached requested V46 updates")
    model = ScaledRetinalGlyphLanguageModelV46(
        model_config,
        retinal_field,
        ...
        for update in range(start_update + 1, arguments["steps"] + 1):
            ...
            for _ in range(arguments["gradient_accumulation"]):
                ...
                with autocast_context(device, arguments["precision"]):
                    ...
                    loss = scaled_retinal_glyph_language_v46_loss(
                        model,
                        prediction,
                        batch["target"],
            ...
            if should_save:
                ...
                payload = _checkpoint(
                    ...
                    retinal_field_receipt=retinal_field_receipt,
            ...
            if stop_requested:
                raise KeyboardInterrupt(
                    f"V46 stopped after checkpointing update {update}"
                )
    finally:
