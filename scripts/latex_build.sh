...
if pdflatex --help 2>&1 | grep -q -- "-aux-directory"; then
  ...
  if ! pdflatex -interaction=nonstopmode -halt-on-error -aux-directory="$AUX_DIR" "$TEX_BASE.tex"; then
    echo "pdflatex failed; trying xelatex..." >&2
    xelatex -interaction=nonstopmode -halt-on-error -aux-directory="$AUX_DIR" "$TEX_BASE.tex"
  fi
  if ! pdflatex -interaction=nonstopmode -halt-on-error -aux-directory="$AUX_DIR" "$TEX_BASE.tex"; then
    echo "second pdflatex pass failed; trying xelatex again..." >&2
    xelatex -interaction=nonstopmode -halt-on-error -aux-directory="$AUX_DIR" "$TEX_BASE.tex"
  fi
else
  # TeX Live fallback: put everything in AUX_DIR, then move the PDF back
  if ! pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$AUX_DIR" "$TEX_BASE.tex"; then
    echo "pdflatex failed; trying xelatex..." >&2
    xelatex -interaction=nonstopmode -halt-on-error -output-directory="$AUX_DIR" "$TEX_BASE.tex"
  fi
  if ! pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$AUX_DIR" "$TEX_BASE.tex"; then
    echo "second pdflatex pass failed; trying xelatex again..." >&2
    xelatex -interaction=nonstopmode -halt-on-error -output-directory="$AUX_DIR" "$TEX_BASE.tex"
