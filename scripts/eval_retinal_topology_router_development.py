...
AUDIT_ARCHITECTURE = "retinal-topology-router-paired-development-audit-v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit selected V20 field and matched-control checkpoints on the "
...
def _validate_student_contract(
    ...
    if contract.get("route_mode") != route_mode:
        raise ValueError("V20 checkpoint has an inconsistent student route")
    for flag in FORBIDDEN_STUDENT_FLAGS:
        if contract.get(flag) is not False:
            raise ValueError("V20 checkpoint violates the image-only student contract")
    global_detail = contract.get("global_state_enters_detail_path")
    if global_detail is not (route_mode == GLOBAL_CONTROL_ROUTE):
        raise ValueError("V20 checkpoint misstates its detail-path condition")


def validate_pair_metadata(
    candidate: dict[str, Any],
    control: dict[str, Any],
) -> tuple[RetinalTopologyRouterConfig, RetinalTopologyRouterConfig]:
    for name, checkpoint, route_mode in (
        ("candidate", candidate, FIELD_ROUTE),
        ("control", control, GLOBAL_CONTROL_ROUTE),
    ):
        if checkpoint.get("architecture") != ARCHITECTURE:
            raise ValueError(f"{name} is not a V20 topology-router checkpoint")
        if checkpoint.get("route_mode") != route_mode:
            raise ValueError(f"{name} has the wrong V20 route mode")
        if checkpoint.get("smoke_only", False):
            raise ValueError("smoke-only V20 checkpoints cannot produce evidence")
        ...
        if checkpoint.get("selection_rule") != selection_rule(route_mode):
            raise ValueError(f"{name} selection rule differs from V20")
        _validate_student_contract(checkpoint, route_mode)

    if candidate.get("partition") != control.get("partition"):
        raise ValueError("V20 arms use different partitions")
    if candidate.get("pvf_sha256") != control.get("pvf_sha256"):
        raise ValueError("V20 arms use different Predictive Visual Fields")
    for argument in PAIRED_ARGUMENT_NAMES:
        ...
        if candidate_value != control_value:
            raise ValueError(f"V20 arms differ on fixed argument {argument!r}")

    candidate_config = retinal_topology_router_config_from_payload(
        candidate["planner_config"]
    )
    control_config = retinal_topology_router_config_from_payload(
        control["planner_config"]
    )
    candidate_base = dict(candidate["planner_config"])
    control_base = dict(control["planner_config"])
    candidate_base.pop("route_mode", None)
    control_base.pop("route_mode", None)
    if candidate_base != control_base:
        raise ValueError("V20 arms do not have equal architecture shapes")
    if candidate_config.route_mode != FIELD_ROUTE:
        raise ValueError("candidate planner configuration is not field-primary")
    if control_config.route_mode != GLOBAL_CONTROL_ROUTE:
        raise ValueError("control planner configuration is not global-detail")
...
def _load_planner(
    ...
    planner = RetinalTopologyRouter(config)
    planner.load_state_dict(checkpoint["planner"], strict=True)
    return planner.to(device).eval().requires_grad_(False)


def _parameter_count(planner: RetinalTopologyRouter) -> int:
    return sum(parameter.numel() for parameter in planner.parameters())
...
def main() -> None:
    ...
    if evaluation_path.exists():
        raise FileExistsError(f"refusing to overwrite V20 audit: {evaluation_path}")
    ...
    candidate = _load_planner(candidate_checkpoint, candidate_config, device)
    control = _load_planner(control_checkpoint, control_config, device)
    ...
    if candidate_parameters != control_parameters:
        raise ValueError("V20 candidate and control parameter counts differ")

    pvf_path = args.pvf_checkpoint or candidate_checkpoint["pvf_checkpoint"]
    if file_sha256(pvf_path) != EXPECTED_PVF_SHA256:
        raise ValueError("Predictive Visual Field bytes differ from the V20 receipt")
    ...
    if partition != candidate_checkpoint["partition"]:
        raise ValueError("paired audit partition differs from the V20 receipt")
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            for route_mode, planner in (
                (FIELD_ROUTE, candidate),
                (GLOBAL_CONTROL_ROUTE, control),
            ):
    ...
    for route_mode, aggregate in aggregates.items():
        metrics[route_mode] = {
            key: float(value)
            for key, value in summarize_retinal_topology_router_trace(
                aggregate,
    ...
    paired_report = paired_gate_report(
        metrics[FIELD_ROUTE],
        metrics[GLOBAL_CONTROL_ROUTE],
        candidate_parameters=candidate_parameters,
        control_parameters=control_parameters,
    )
    candidate_passed = all(arm_gate_reports[FIELD_ROUTE].values())
    control_passed = all(arm_gate_reports[GLOBAL_CONTROL_ROUTE].values())
    paired_passed = candidate_passed and control_passed and all(paired_report.values())

    output.mkdir(parents=True, exist_ok=True)
    sample_count = min(
        args.sample_count,
        int(aggregates[FIELD_ROUTE]["target_ink"].shape[0]),
    )
    sample_artifacts = {
        ...
        for route_mode in (FIELD_ROUTE, GLOBAL_CONTROL_ROUTE)
    }
    examples = int(aggregates[FIELD_ROUTE]["target_ink"].shape[0])
    payload: dict[str, Any] = {
        ...
        "arm_automatic_development_gate_passed": {
            FIELD_ROUTE: candidate_passed,
            GLOBAL_CONTROL_ROUTE: control_passed,
        },
