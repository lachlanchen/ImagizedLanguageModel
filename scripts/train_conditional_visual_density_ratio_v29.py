...
SOURCE_FILES = (
    "ilm/visual_lm/conditional_visual_density_ratio.py",
    "ilm/visual_lm/conditional_visual_density_ratio_data.py",
    "ilm/visual_lm/conditional_visual_density_ratio_training.py",
    "scripts/train_conditional_visual_density_ratio_v29.py",
    "scripts/eval_conditional_visual_density_ratio_v29.py",
)
FIXED_MODEL_CONFIG = ConditionalVisualDensityRatioConfig()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train preregistered V29 conditional visual density ratio."
    )
    parser.add_argument("--manifest", default=DEFAULT_MANIFEST)
    parser.add_argument("--v28-checkpoint", default=DEFAULT_V28_CHECKPOINT)
    parser.add_argument("--resume", default=None)
    parser.add_argument(
        "--out", default="artifacts/conditional_visual_density_ratio_v29_evidence"
    )
    ...
    parser.add_argument("--evidence-learning-rate", type=float, default=3e-4)
    ...
    parser.add_argument("--dataset-seed", type=int, default=20260921)
    parser.add_argument("--pair-seed", type=int, default=20260922)
    parser.add_argument("--candidate-bank-seed", type=int, default=20260923)
...
def _require_fixed_evidence_arguments(args: argparse.Namespace) -> None:
    ...
    for name, expected in FIXED_OPTIMIZATION.items():
        if getattr(args, name) != expected:
            raise ValueError(
                f"V29 evidence requires --{name.replace('_', '-')}={expected}"
            )
...
def _effective_arguments(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not 1 <= args.smoke_steps <= 20:
        raise ValueError("V29 smoke mode permits 1-20 updates")
    output.update(
        {
...
def load_v28_initialization(
    ...
    *,
    ...
    if require_expected_hash and digest != EXPECTED_V28_SHA256:
        raise ValueError(
            f"V29 requires V28 checkpoint {EXPECTED_V28_SHA256}, got {digest}"
        )
    source = torch.load(path, map_location="cpu", weights_only=False)
    if source.get("architecture") != "dense-visual-future-energy-v28":
        raise ValueError("V29 initialization source is not V28")
    ...
    for name in (
        ...
        if source_config.get(name) != getattr(model.config, name):
            raise ValueError(f"V29 V28 source differs for {name}")
    retina_source = source.get("initialization", {}).get("sha256")
    if require_expected_hash and retina_source != EXPECTED_RETINA_SHA256:
        raise ValueError("V29 V28 source has the wrong retina receipt")
    loaded = model.load_v28_backbone_state(source["model"])
    return {
        "route": "v28-conditional-backbone-transfer",
        "checkpoint": str(path),
        "sha256": digest,
        "expected_sha256": EXPECTED_V28_SHA256,
...
def _candidate_manifest(
    ...
    full = conditional_visual_candidate_bank_receipt(
        bank, include_host_forms=True
    )
    ...
    *,
    ...
    for images in bank.images:
        ...
        for start in range(0, images.shape[0], chunk_size):
            ...
            semantic_parts.append(semantic.detach())
        views.append((torch.cat(raw_parts), torch.cat(semantic_parts)))
    if len(views) != 2:
        raise ValueError("V29 training bank requires two views")
...
def _to_device_natural(
    ...
    targets = canonical_target_indices(
        raw["canonical_target"], bank.canonical
    ...
    *,
    ...
    for name, parameter in model.named_parameters():
        ...
        if name.startswith(context_names):
            ...
            evidence.append(parameter)
    if not context or not evidence:
        raise ValueError("V29 optimizer requires context and evidence parameters")
        ...
        [
            {
                ...
                "group_name": "v28_context",
            },
            {
                "params": evidence,
                "lr": evidence_learning_rate,
                "base_lr": evidence_learning_rate,
                "group_name": "candidate_evidence",
            },
        ],
        betas=(0.9, 0.95),
...
def _protocol_receipt(
    *,
    ...
    return {
        ...
        "expected_v28_sha256": EXPECTED_V28_SHA256,
        "expected_retina_sha256": EXPECTED_RETINA_SHA256,
        "fixed_model_config": conditional_visual_density_ratio_config_payload(
            FIXED_MODEL_CONFIG
        ),
        ...
        "fixed_order_margin": V29_ORDER_MARGIN,
...
def _checkpoint_payload(
    ...
    *,
    ...
    if any("bank" in name.lower() for name in model_state):
        raise RuntimeError("V29 model state unexpectedly contains a bank")
    return {
        "architecture": ARCHITECTURE,
        "step": step,
        "smoke_only": smoke_only,
        "exploratory": exploratory,
        "model_config": conditional_visual_density_ratio_config_payload(model.config),
        "render_config": conditional_visual_render_config_payload(
            FIXED_RENDER_CONFIG
        ),
        "model": model_state,
        ...
        "data_boundary": conditional_visual_data_boundary_receipt(),
        "model_boundary": conditional_visual_density_ratio_boundary_receipt(
            model.config
        ),
    ...
    *,
    ...
    for name in (
        ...
        if previous.get(name) != arguments[name]:
            raise ValueError(f"V29 resume differs for {name}")
    if not checkpoint.get("resumable"):
        raise ValueError("final V29 checkpoints are not resumable")
    if checkpoint.get("optimizer") is None or checkpoint.get("rng_state") is None:
        raise ValueError("V29 resume lacks optimizer or RNG state")
...
def train(
    ...
    *,
    ...
    if not 0 <= start_step < steps:
        raise ValueError("V29 start step must precede the final step")
    model.train()
    if _trainable_parameters(model) > 22_000_000:
        raise ValueError("V29 exceeds the preregistered trainable parameter cap")
    if sum(parameter.numel() for parameter in model.parameters()) > 25_000_000:
        raise ValueError("V29 exceeds the preregistered total parameter cap")
    optimizer = build_optimizer(
        model,
        evidence_learning_rate=float(arguments["evidence_learning_rate"]),
    ...
    try:
        for step in range(start_step + 1, steps + 1):
            ...
            evidence_lr = scheduled_lr(
                step,
                base=float(arguments["evidence_learning_rate"]),
                warmup=int(arguments["warmup"]),
                total=steps,
                minimum_ratio=float(arguments["minimum_lr_ratio"]),
            )
            for group in optimizer.param_groups:
                group["lr"] = (
                    context_lr
                    if group["group_name"] == "v28_context"
                    else evidence_lr
                )
            ...
            final_metrics.update(
                {
                    ...
                    "evidence_learning_rate": evidence_lr,
            ...
            if stop_requested:
                raise KeyboardInterrupt(
                    f"V29 stopped after checkpointing update {step}"
                )
    finally:
...
def main() -> None:
    ...
    if not exploratory and (device.type != "cuda" or arguments["precision"] != "bf16"):
        raise ValueError("V29 evidence requires CUDA BF16 execution")
    if not exploratory and device.index not in (None, 0):
        raise ValueError("V29 evidence requires CUDA device 0")
    if device.type == "cuda":
    ...
    statistics = build_v29_candidate_statistics(
        records, bank_size=int(arguments["candidate_bank_size"])
    )
    bank = build_v29_candidate_bank(
    ...
    if arguments["resume"]:
        ...
        model = ConditionalVisualDensityRatioModel(FIXED_MODEL_CONFIG)
        initialization = load_v28_initialization(
            model,
            arguments["v28_checkpoint"],
    ...
    report.update(
        {
            ...
            "candidate_bank_manifest": candidate_bank_receipt,
            "device": str(device),
        }
    )
    atomic_write_json(report, output / "development_audit.json")
