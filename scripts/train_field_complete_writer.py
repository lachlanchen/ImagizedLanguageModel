#!/usr/bin/env python3
from __future__ import annotations

import argparse
import json
import random
import signal
import time
from pathlib import Path
from typing import Any

import numpy as np
import torch
from torch.utils.data import DataLoader

from ilm.visual_lm.field_complete_writer import (
    FIELD_COMPLETE_ROUTE,
    ROUTE_MODES,
    TILED_GLOBAL_CONTROL_ROUTE,
    FieldCompleteWriter,
    FieldCompleteWriterConfig,
    evaluate_field_complete_writer_batch,
    field_complete_writer_config_from_payload,
    field_complete_writer_config_payload,
    field_complete_writer_loss,
    summarize_field_complete_writer_trace,
)
from ilm.visual_lm.ink_jepa_data import (
    RetinalRenderConfig,
    load_visual_grammar_manifest,
)
from ilm.visual_lm.predictive_visual_field import PredictiveVisualField
from ilm.visual_lm.saccade_data import (
    SaccadeSequenceSpec,
    VisualSaccadeDataset,
    visual_saccade_collate,
)
from scripts.train_retinal_topology_router import save_sample_grid
from scripts.train_visual_motor_plan import partition_receipt, partition_records
from scripts.train_visual_state_actuator import (
    append_jsonl,
    atomic_save,
    autocast_context,
    choose_device,
    file_sha256,
    load_pvf,
    scheduled_lr,
    select_examples,
)


ARCHITECTURE = "field-complete-writer-v1"
PARTITION_SALT = "field-complete-writer-v21"
EXPECTED_PVF_SHA256 = (
    "90791001203640f0de66316cf2e30b3e2c588480fef0e3d9d4f6283ba043ecbe"
)
EXPECTED_PARAMETERS = 582_336
PROTOCOL_DOCUMENT = "references/field_complete_writer_v21_protocol.md"
CANDIDATE_SELECTION_RULE = {
    "primary": "maximize development overall correct pixel F1",
    "tie_breakers": [
        "maximize development dense correct pixel F1",
        "prefer the earlier step",
    ],
    "requirements": [
        "correct_pixel_f1 > 0.66",
        "correct_pixel_f1_simple > 0.58",
        "correct_pixel_f1_medium > 0.60",
        "correct_pixel_f1_dense > 0.70",
        "correct_pixel_f1_dense - field_shuffled_pixel_f1_dense > 0.15",
        "correct_pixel_f1_dense - zero_field_pixel_f1_dense > 0.20",
        "correct_identity_top1 > 0.74",
        "correct_identity_top1 > both_shuffled_identity_top1",
        "correct_target_cosine > 0.82",
        "correct_target_cosine > both_shuffled_target_cosine",
        "field_condition_pixel_l1 > 0.12",
        "zero_field_condition_pixel_l1 > 0.15",
        "occlusion_pixel_change > 0.03",
        "occlusion_locality > 0.95",
        "style_copy_cosine < 0.30",
        "semantic_target_pixel_l1 > 0.05",
        "basis_dc_leakage_max == 0.0",
        "zero_source_cell_variation_max < 1e-6",
        "detail_block_mean_abs_max < 5e-6",
        "decomposition_error_max < 1e-6",
        "frozen_images_instantiated == 0",
    ],
    "paired_control_gate_required_before_human_review": True,
    "blinded_readability_review_required_before_frozen_evaluation": True,
    "frozen_partition_read_during_selection": False,
}
CONTROL_SELECTION_RULE = {
    "primary": "maximize development overall correct pixel F1",
    "tie_breakers": [
        "maximize development dense correct pixel F1",
        "prefer the earlier step",
    ],
    "requirements": [
        "basis_dc_leakage_max == 0.0",
        "basis_gram_error_max < 1e-6",
        "zero_source_cell_variation_max < 1e-6",
        "detail_block_mean_abs_max < 5e-6",
        "decomposition_error_max < 1e-6",
        "frozen_images_instantiated == 0",
    ],
    "information_ablation_only": True,
    "frozen_partition_read_during_selection": False,
}
FIXED_MODEL_ARGUMENTS = {
    "style_dim": 64,
    "style_base_channels": 32,
    "hidden_channels": 128,
    "context_dim": 128,
    "pointwise_blocks": 3,
    "dropout": 0.05,
}
FIXED_LOSS_ARGUMENTS = {
    "stroke_weight": 4.0,
    "dice_weight": 1.0,
    "pixel_l1_weight": 0.50,
    "edge_weight": 0.25,
    "identity_weight": 0.10,
    "contrastive_weight": 0.05,
    "field_margin_weight": 0.25,
    "field_margin": 0.05,
    "zero_margin_weight": 0.25,
    "zero_margin": 0.05,
    "coarse_weight": 0.50,
    "duplicate_similarity": 0.90,
    "logit_scale": 12.5,
}
FIXED_OPTIMIZATION_ARGUMENTS = {
    "lr": 3e-4,
    "minimum_lr_ratio": 0.10,
    "warmup_steps": 100,
    "weight_decay": 0.03,
    "gradient_clip": 1.0,
    "seed": 20260821,
}
FIXED_EVIDENCE_ARGUMENTS = {
    "holdout_fraction": 0.06,
    "development_share": 0.50,
    "sequence_length": 24,
    "positions_per_sequence": 4,
    "samples_per_epoch": 100_000,
    "development_samples": 128,
    "maximum_steps": 1_600,
    "batch_size": 32,
    "num_workers": 8,
    "precision": "bf16",
    "validate_every": 200,
    "validation_batches": 4,
    "save_every": 200,
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train one preregistered V21 field-complete writer arm from "
            "continuous image-derived global and local state."
        )
    )
    parser.add_argument("--pvf-checkpoint", required=True)
    parser.add_argument("--route-mode", choices=ROUTE_MODES, required=True)
    parser.add_argument("--resume", default=None)
    parser.add_argument("--smoke", action="store_true")
    parser.add_argument(
        "--manifest",
        default="data/visual_grammar/chinese_wikisource_public_domain.jsonl",
    )
    parser.add_argument("--out", default="artifacts/field_complete_writer_v21")
    parser.add_argument("--partition-salt", default=PARTITION_SALT)
    parser.add_argument("--holdout-fraction", type=float, default=0.06)
    parser.add_argument("--development-share", type=float, default=0.50)
    parser.add_argument("--sequence-length", type=int, default=24)
    parser.add_argument("--positions-per-sequence", type=int, default=4)
    parser.add_argument("--samples-per-epoch", type=int, default=100_000)
    parser.add_argument("--development-samples", type=int, default=128)
    parser.add_argument("--style-dim", type=int, default=64)
    parser.add_argument("--style-base-channels", type=int, default=32)
    parser.add_argument("--hidden-channels", type=int, default=128)
    parser.add_argument("--context-dim", type=int, default=128)
    parser.add_argument("--pointwise-blocks", type=int, default=3)
    parser.add_argument("--dropout", type=float, default=0.05)
    parser.add_argument("--stroke-weight", type=float, default=4.0)
    parser.add_argument("--dice-weight", type=float, default=1.0)
    parser.add_argument("--pixel-l1-weight", type=float, default=0.50)
    parser.add_argument("--edge-weight", type=float, default=0.25)
    parser.add_argument("--identity-weight", type=float, default=0.10)
    parser.add_argument("--contrastive-weight", type=float, default=0.05)
    parser.add_argument("--field-margin-weight", type=float, default=0.25)
    parser.add_argument("--field-margin", type=float, default=0.05)
    parser.add_argument("--zero-margin-weight", type=float, default=0.25)
    parser.add_argument("--zero-margin", type=float, default=0.05)
    parser.add_argument("--coarse-weight", type=float, default=0.50)
    parser.add_argument("--duplicate-similarity", type=float, default=0.90)
    parser.add_argument("--logit-scale", type=float, default=12.5)
    parser.add_argument("--epochs", type=int, default=20)
    parser.add_argument("--maximum-steps", type=int, default=1_600)
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--num-workers", type=int, default=8)
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--minimum-lr-ratio", type=float, default=0.10)
    parser.add_argument("--warmup-steps", type=int, default=100)
    parser.add_argument("--weight-decay", type=float, default=0.03)
    parser.add_argument("--gradient-clip", type=float, default=1.0)
    parser.add_argument("--precision", choices=("fp32", "fp16", "bf16"), default="bf16")
    parser.add_argument("--device", default="auto")
    parser.add_argument("--seed", type=int, default=20260821)
    parser.add_argument("--log-every", type=int, default=20)
    parser.add_argument("--validate-every", type=int, default=200)
    parser.add_argument("--validation-batches", type=int, default=4)
    parser.add_argument("--save-every", type=int, default=200)
    parser.add_argument("--sample-count", type=int, default=8)
    return parser.parse_args()


def seed_everything(seed: int) -> None:
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)


def _require_fixed_arguments(args: argparse.Namespace) -> None:
    if args.partition_salt != PARTITION_SALT:
        raise ValueError(f"V21 requires partition salt {PARTITION_SALT!r}")
    for group in (
        FIXED_MODEL_ARGUMENTS,
        FIXED_LOSS_ARGUMENTS,
        FIXED_OPTIMIZATION_ARGUMENTS,
    ):
        for name, expected in group.items():
            if getattr(args, name) != expected:
                raise ValueError(f"V21 requires --{name.replace('_', '-')}={expected}")
    if args.smoke:
        if args.maximum_steps > 20:
            raise ValueError("V21 smoke mode is limited to 20 optimization steps")
        return
    for name, expected in FIXED_EVIDENCE_ARGUMENTS.items():
        if getattr(args, name) != expected:
            raise ValueError(f"V21 evidence requires --{name.replace('_', '-')}={expected}")


def selection_rule(route_mode: str) -> dict[str, Any]:
    if route_mode == FIELD_COMPLETE_ROUTE:
        return CANDIDATE_SELECTION_RULE
    if route_mode == TILED_GLOBAL_CONTROL_ROUTE:
        return CONTROL_SELECTION_RULE
    raise ValueError(f"unknown route mode {route_mode!r}")


def selection_gate_report(
    metrics: dict[str, float],
    route_mode: str,
) -> dict[str, bool]:
    structural = {
        "basis_dc_invariant": metrics["basis_dc_leakage_max"] == 0.0,
        "basis_orthogonality": metrics["basis_gram_error_max"] < 1e-6,
        "zero_source_repeated_cells": (
            metrics["zero_source_cell_variation_max"] < 1e-6
        ),
        "detail_invariant": metrics["detail_block_mean_abs_max"] < 5e-6,
        "decomposition_invariant": metrics["decomposition_error_max"] < 1e-6,
        "frozen_bank_sealed": (
            metrics.get("frozen_images_instantiated", 0.0) == 0.0
        ),
    }
    if route_mode == TILED_GLOBAL_CONTROL_ROUTE:
        return structural
    if route_mode != FIELD_COMPLETE_ROUTE:
        raise ValueError(f"unknown route mode {route_mode!r}")
    dense = metrics["correct_pixel_f1_dense"]
    return {
        "overall_pixel_f1": metrics["correct_pixel_f1"] > 0.66,
        "simple_pixel_f1": metrics["correct_pixel_f1_simple"] > 0.58,
        "medium_pixel_f1": metrics["correct_pixel_f1_medium"] > 0.60,
        "dense_pixel_f1": dense > 0.70,
        "dense_field_shuffle_margin": (
            dense - metrics["field_shuffled_pixel_f1_dense"] > 0.15
        ),
        "dense_zero_field_margin": (
            dense - metrics["zero_field_pixel_f1_dense"] > 0.20
        ),
        "identity_top1": (
            metrics["correct_identity_top1"] > 0.74
            and metrics["correct_identity_top1"]
            > metrics["both_shuffled_identity_top1"]
        ),
        "target_cosine": (
            metrics["correct_target_cosine"] > 0.82
            and metrics["correct_target_cosine"]
            > metrics["both_shuffled_target_cosine"]
        ),
        "field_condition_pixel_l1": metrics["field_condition_pixel_l1"] > 0.12,
        "zero_field_condition_pixel_l1": (
            metrics["zero_field_condition_pixel_l1"] > 0.15
        ),
        "occlusion_pixel_change": metrics["occlusion_pixel_change"] > 0.03,
        "occlusion_locality": metrics["occlusion_locality"] > 0.95,
        "style_noncopying": metrics["style_copy_cosine"] < 0.30,
        "semantic_target_pixel_l1": metrics["semantic_target_pixel_l1"] > 0.05,
        **structural,
    }


def selection_eligible(metrics: dict[str, float], route_mode: str) -> bool:
    return all(selection_gate_report(metrics, route_mode).values())


def paired_gate_report(
    candidate: dict[str, float],
    control: dict[str, float],
    *,
    candidate_parameters: int,
    control_parameters: int,
) -> dict[str, bool]:
    return {
        "candidate_overall_gain": (
            candidate["correct_pixel_f1"] - control["correct_pixel_f1"] > 0.20
        ),
        "candidate_dense_gain": (
            candidate["correct_pixel_f1_dense"]
            - control["correct_pixel_f1_dense"]
            > 0.20
        ),
        "candidate_gate_retained": all(
            selection_gate_report(candidate, FIELD_COMPLETE_ROUTE).values()
        ),
        "control_structure_retained": all(
            selection_gate_report(
                control,
                TILED_GLOBAL_CONTROL_ROUTE,
            ).values()
        ),
        "exact_parameter_count": candidate_parameters == control_parameters,
    }


def better_development_candidate(
    candidate: dict[str, Any],
    incumbent: dict[str, Any] | None,
) -> bool:
    if incumbent is None:
        return True
    candidate_key = (
        float(candidate["correct_pixel_f1"]),
        float(candidate["correct_pixel_f1_dense"]),
        -int(candidate["step"]),
    )
    incumbent_key = (
        float(incumbent["correct_pixel_f1"]),
        float(incumbent["correct_pixel_f1_dense"]),
        -int(incumbent["step"]),
    )
    return candidate_key > incumbent_key


def development_snapshot(
    metrics: dict[str, float],
    step: int,
    route_mode: str,
) -> dict[str, Any]:
    keys = (
        "correct_pixel_f1",
        "correct_pixel_f1_simple",
        "correct_pixel_f1_medium",
        "correct_pixel_f1_dense",
        "field_shuffled_pixel_f1_dense",
        "global_shuffled_pixel_f1_dense",
        "both_shuffled_pixel_f1_dense",
        "zero_field_pixel_f1_dense",
        "correct_identity_top1",
        "both_shuffled_identity_top1",
        "correct_target_cosine",
        "both_shuffled_target_cosine",
        "condition_pixel_l1",
        "field_condition_pixel_l1",
        "zero_field_condition_pixel_l1",
        "global_condition_pixel_l1",
        "occlusion_pixel_change",
        "occlusion_locality",
        "style_copy_cosine",
        "semantic_target_pixel_l1",
        "basis_dc_leakage_max",
        "basis_gram_error_max",
        "zero_source_cell_variation_max",
        "detail_block_mean_abs_max",
        "decomposition_error_max",
        "detail_rms",
    )
    return {
        "step": step,
        "route_mode": route_mode,
        **{key: float(metrics[key]) for key in keys},
    }


def config_from_pvf(
    checkpoint: dict[str, Any],
    args: argparse.Namespace,
) -> FieldCompleteWriterConfig:
    pvf_config = checkpoint["model_config"]
    fovea_size = int(pvf_config["fovea_size"])
    visual_dim = int(pvf_config["visual_dim"])
    spatial_channels = int(pvf_config["retina_base_channels"]) * 3
    return FieldCompleteWriterConfig(
        fovea_size=fovea_size,
        field_size=fovea_size // 8,
        visual_dim=visual_dim,
        spatial_channels=spatial_channels,
        style_dim=args.style_dim,
        style_base_channels=args.style_base_channels,
        hidden_channels=args.hidden_channels,
        context_dim=args.context_dim,
        pointwise_blocks=args.pointwise_blocks,
        dropout=args.dropout,
        route_mode=args.route_mode,
    )


def validate_resume_checkpoint(
    checkpoint: dict[str, Any],
    *,
    expected_partition: dict[str, Any],
    expected_pvf_sha256: str,
    expected_config: FieldCompleteWriterConfig,
    smoke: bool,
) -> None:
    if checkpoint.get("architecture") != ARCHITECTURE:
        raise ValueError("resume is not a V21 field-complete checkpoint")
    if checkpoint.get("partition") != expected_partition:
        raise ValueError("resume uses a different V21 partition")
    if checkpoint.get("pvf_sha256") != expected_pvf_sha256:
        raise ValueError("resume uses a different Predictive Visual Field")
    restored = field_complete_writer_config_from_payload(
        checkpoint["writer_config"]
    )
    if restored != expected_config:
        raise ValueError("resume uses a different field-complete configuration")
    if bool(checkpoint.get("smoke_only", False)) != smoke:
        raise ValueError("resume cannot cross the smoke/evidence boundary")
    if checkpoint.get("route_mode") != expected_config.route_mode:
        raise ValueError("resume uses a different V21 route mode")


@torch.no_grad()
def validate(
    pvf: PredictiveVisualField,
    writer: FieldCompleteWriter,
    loader: DataLoader,
    *,
    device: torch.device,
    args: argparse.Namespace,
    step: int,
    sample_root: Path,
) -> dict[str, float]:
    writer.eval()
    totals: dict[str, float] = {}
    traces: dict[str, list[torch.Tensor]] = {}
    first_trace: dict[str, torch.Tensor] | None = None
    examples = 0
    generator = torch.Generator(device=device).manual_seed(
        args.seed + step * 100_003
    )
    for batch_index, batch in enumerate(loader):
        if batch_index >= args.validation_batches:
            break
        target, semantic, style = select_examples(
            batch,
            positions_per_sequence=args.positions_per_sequence,
            device=device,
            generator=generator,
        )
        with autocast_context(device, args.precision):
            loss, training_metrics, _ = field_complete_writer_loss(
                writer,
                pvf.retina,
                target,
                semantic,
                style,
                stroke_weight=args.stroke_weight,
                dice_weight=args.dice_weight,
                pixel_l1_weight=args.pixel_l1_weight,
                edge_weight=args.edge_weight,
                identity_weight=args.identity_weight,
                contrastive_weight=args.contrastive_weight,
                field_margin_weight=args.field_margin_weight,
                field_margin=args.field_margin,
                zero_margin_weight=args.zero_margin_weight,
                zero_margin=args.zero_margin,
                coarse_weight=args.coarse_weight,
                duplicate_similarity=args.duplicate_similarity,
                logit_scale=args.logit_scale,
            )
            _, trace = evaluate_field_complete_writer_batch(
                writer,
                pvf.retina,
                target,
                semantic,
                style,
                duplicate_similarity=args.duplicate_similarity,
                logit_scale=args.logit_scale,
            )
        if first_trace is None:
            first_trace = {
                key: value.detach().cpu()
                for key, value in trace.items()
                if key != "detail_basis"
            }
        for key, value in trace.items():
            if key != "detail_basis":
                traces.setdefault(key, []).append(value.detach())
        batch_examples = target.shape[0]
        for key, value in {"loss": loss.detach(), **training_metrics}.items():
            totals[key] = totals.get(key, 0.0) + float(value) * batch_examples
        examples += batch_examples

    if examples == 0:
        raise ValueError("development loader yielded no V21 examples")
    aggregate_trace = {key: torch.cat(values) for key, values in traces.items()}
    aggregate_trace["detail_basis"] = writer.detail_basis.detach()
    causal_metrics = summarize_field_complete_writer_trace(
        aggregate_trace,
        field_size=writer.config.field_size,
        duplicate_similarity=args.duplicate_similarity,
        logit_scale=args.logit_scale,
    )
    if first_trace is not None:
        save_sample_grid(
            first_trace,
            sample_root / f"step_{step:07d}.png",
            args.sample_count,
        )
    writer.train()
    report = {
        **{f"plan_{key}": value / examples for key, value in totals.items()},
        **{key: float(value) for key, value in causal_metrics.items()},
        "examples": float(examples),
        "retrieval_candidates": float(aggregate_trace["intended_visual"].shape[0]),
        "frozen_images_instantiated": 0.0,
    }
    report["selection_eligible"] = float(
        selection_eligible(report, writer.config.route_mode)
    )
    return report


def checkpoint_payload(
    writer: FieldCompleteWriter,
    optimizer: torch.optim.Optimizer,
    *,
    args: argparse.Namespace,
    pvf_checkpoint: dict[str, Any],
    pvf_sha256: str,
    partition: dict[str, Any],
    best_development: dict[str, Any] | None,
    epoch: int,
    step: int,
    elapsed_seconds: float,
) -> dict[str, Any]:
    return {
        "architecture": ARCHITECTURE,
        "route_mode": writer.config.route_mode,
        "writer_config": field_complete_writer_config_payload(writer.config),
        "writer": writer.state_dict(),
        "optimizer": optimizer.state_dict(),
        "pvf_checkpoint": args.pvf_checkpoint,
        "pvf_sha256": pvf_sha256,
        "pvf_architecture": pvf_checkpoint["architecture"],
        "pvf_step": int(pvf_checkpoint.get("global_step", 0)),
        "pvf_model_config": pvf_checkpoint["model_config"],
        "render_config": pvf_checkpoint["render_config"],
        "partition": partition,
        "epoch": epoch,
        "global_step": step,
        "elapsed_seconds": elapsed_seconds,
        "arguments": vars(args),
        "selection_rule": selection_rule(writer.config.route_mode),
        "best_development": best_development,
        "smoke_only": args.smoke,
        "paired_control_gate_passed": False,
        "blinded_readability_gate_passed": False,
        "frozen_images_instantiated_during_training": False,
        "student_contract": {
            "input": (
                "continuous global retinal state, continuous 4x4 retinal field, "
                "and a separate continuous style image"
            ),
            "output": "continuous 4x4 grid of complete 8x8 ink-logit patches",
            "route_mode": writer.config.route_mode,
            "global_state_enters_uniform_modulation": True,
            "global_state_enters_spatial_source": (
                writer.config.route_mode == TILED_GLOBAL_CONTROL_ROUTE
            ),
            "learned_or_fixed_position_input": False,
            "spatial_cell_mixing": False,
            "target_spatial_pixels_enter_condition": False,
            "student_received_token_ids": False,
            "student_received_unicode_ids": False,
            "student_received_ocr": False,
            "student_received_character_labels": False,
            "student_used_visual_codebook": False,
            "student_used_candidate_classifier": False,
            "student_used_external_language_model": False,
            "retina_trainable": False,
        },
        "patch_basis": {
            "name": "walsh-hadamard-zero-dc",
            "shape": list(writer.detail_basis.shape),
            "trainable": False,
            "dc_leakage_max": float(writer.detail_basis.sum(dim=0).abs().max()),
        },
    }


def _write_selection_receipt(
    output: Path,
    route_mode: str,
    selected: dict[str, Any],
) -> None:
    reason = (
        "paired control and blinded readability gates remain"
        if route_mode == FIELD_COMPLETE_ROUTE
        else "information-ablation control cannot open frozen data"
    )
    (output / "development_selection.json").write_text(
        json.dumps(
            {
                "route_mode": route_mode,
                "selection_rule": selection_rule(route_mode),
                "selected": selected,
                "arm_development_gate_passed": True,
                "paired_control_gate_passed": False,
                "blinded_readability_gate_passed": False,
                "frozen_evaluation_permitted": False,
                "reason": reason,
            },
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
        )
        + "\n",
        encoding="utf-8",
    )


def main() -> None:
    args = parse_args()
    _require_fixed_arguments(args)
    seed_everything(args.seed)
    device = choose_device(args.device)
    if not args.smoke and device.type != "cuda":
        raise ValueError("V21 evidence runs require one CUDA GPU")
    if device.type == "cuda":
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.allow_tf32 = True

    output = Path(args.out)
    output.mkdir(parents=True, exist_ok=True)
    log_path = output / "training.jsonl"
    pvf, pvf_checkpoint = load_pvf(args.pvf_checkpoint, device)
    pvf_sha256 = file_sha256(args.pvf_checkpoint)
    if pvf_sha256 != EXPECTED_PVF_SHA256:
        raise ValueError("V21 requires the preregistered V16 PVF checkpoint")

    records = load_visual_grammar_manifest(args.manifest)
    train_records, development_records, frozen_records = partition_records(
        records,
        salt=args.partition_salt,
        holdout_fraction=args.holdout_fraction,
        development_share=args.development_share,
    )
    partition = partition_receipt(
        train_records,
        development_records,
        frozen_records,
        salt=args.partition_salt,
        holdout_fraction=args.holdout_fraction,
        development_share=args.development_share,
    )
    config = config_from_pvf(pvf_checkpoint, args)
    writer = FieldCompleteWriter(config).to(device)
    parameters = sum(parameter.numel() for parameter in writer.parameters())
    if parameters != EXPECTED_PARAMETERS:
        raise ValueError(
            f"V21 requires exactly {EXPECTED_PARAMETERS} trainable parameters"
        )
    protocol = {
        "architecture": ARCHITECTURE,
        "experiment": "V21 field-complete visual writer",
        "protocol_document": PROTOCOL_DOCUMENT,
        "route_mode": args.route_mode,
        "partition": partition,
        "pvf_sha256": pvf_sha256,
        "writer_config": field_complete_writer_config_payload(config),
        "expected_parameters": EXPECTED_PARAMETERS,
        "selection_rule": selection_rule(args.route_mode),
        "paired_selection_rule": {
            "candidate_overall_gain": "> 0.20",
            "candidate_dense_gain": "> 0.20",
            "candidate_gate_retained": True,
            "control_structure_retained": True,
            "parameter_counts": "exactly equal",
        },
        "causal_branches": [
            "correct",
            "field_shuffled",
            "global_shuffled",
            "both_shuffled",
            "zero_field",
            "four quadrant occlusions",
        ],
        "smoke_only": args.smoke,
        "frozen_evaluation_status": (
            "forbidden until candidate, paired-control, and blinded gates pass"
        ),
    }
    protocol_path = output / "preregistered_protocol.json"
    if protocol_path.exists():
        existing = json.loads(protocol_path.read_text(encoding="utf-8"))
        if existing != protocol:
            raise ValueError("existing preregistered V21 protocol differs")
    else:
        protocol_path.write_text(
            json.dumps(protocol, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
    (output / "partition.json").write_text(
        json.dumps(partition, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )

    trainable = [parameter for parameter in writer.parameters() if parameter.requires_grad]
    optimizer = torch.optim.AdamW(
        trainable,
        lr=args.lr,
        weight_decay=args.weight_decay,
        betas=(0.9, 0.95),
    )
    scaler = torch.amp.GradScaler(
        "cuda",
        enabled=device.type == "cuda" and args.precision == "fp16",
    )
    global_step = 0
    start_epoch = 0
    elapsed_before = 0.0
    best_development: dict[str, Any] | None = None
    if args.resume:
        resume = torch.load(args.resume, map_location="cpu", weights_only=False)
        validate_resume_checkpoint(
            resume,
            expected_partition=partition,
            expected_pvf_sha256=pvf_sha256,
            expected_config=config,
            smoke=args.smoke,
        )
        writer.load_state_dict(resume["writer"], strict=True)
        writer.to(device)
        optimizer.load_state_dict(resume["optimizer"])
        global_step = int(resume.get("global_step", 0))
        start_epoch = int(resume.get("epoch", 0))
        elapsed_before = float(resume.get("elapsed_seconds", 0.0))
        best_development = resume.get("best_development")

    render_config = RetinalRenderConfig(**pvf_checkpoint["render_config"])
    spec = SaccadeSequenceSpec(
        sequence_length=args.sequence_length,
        fovea_size=pvf.config.fovea_size,
    )
    train_dataset = VisualSaccadeDataset(
        train_records,
        render_config=render_config,
        spec=spec,
        split="all",
        length=args.samples_per_epoch,
        seed=args.seed,
    )
    development_dataset = VisualSaccadeDataset(
        development_records,
        render_config=render_config,
        spec=spec,
        split="all",
        length=args.development_samples,
        seed=args.seed + 1_000_003,
    )
    loader_options = {
        "batch_size": args.batch_size,
        "num_workers": args.num_workers,
        "pin_memory": device.type == "cuda",
        "persistent_workers": False,
        "collate_fn": visual_saccade_collate,
    }
    train_loader = DataLoader(
        train_dataset,
        shuffle=True,
        drop_last=True,
        **loader_options,
    )
    development_loader = DataLoader(
        development_dataset,
        shuffle=False,
        drop_last=False,
        **loader_options,
    )

    planned_steps = args.maximum_steps or args.epochs * max(1, len(train_loader))
    startup = {
        "stage": "startup",
        "architecture": ARCHITECTURE,
        "route_mode": args.route_mode,
        "parameters": parameters,
        "trainable_parameters": sum(parameter.numel() for parameter in trainable),
        "frozen_pvf_parameters": sum(parameter.numel() for parameter in pvf.parameters()),
        "classifier_parameters": 0,
        "token_embedding_parameters": 0,
        "position_parameters": 0,
        "device": str(device),
        "planned_steps": planned_steps,
        "sequence_length": args.sequence_length,
        "positions_per_sequence": args.positions_per_sequence,
        "partition": partition,
        "pvf_sha256": pvf_sha256,
        "selection_rule": selection_rule(args.route_mode),
        "smoke_only": args.smoke,
    }
    print(json.dumps(startup), flush=True)
    append_jsonl(log_path, startup)
    if device.type == "cuda":
        torch.cuda.reset_peak_memory_stats(device)

    stop_requested = False

    def request_stop(signum: int, frame: object) -> None:
        del frame
        nonlocal stop_requested
        stop_requested = True
        print(
            json.dumps(
                {"stage": "signal", "signal": signum, "action": "save_then_stop"}
            ),
            flush=True,
        )

    signal.signal(signal.SIGINT, request_stop)
    signal.signal(signal.SIGTERM, request_stop)
    started = time.perf_counter()
    interval_started = started
    running: dict[str, float] = {}
    running_examples = 0
    generator = torch.Generator(device=device).manual_seed(
        args.seed + global_step * 31
    )
    epoch = start_epoch
    writer.train()
    for epoch in range(start_epoch, args.epochs):
        train_dataset.set_epoch(epoch)
        for batch in train_loader:
            if stop_requested or global_step >= planned_steps:
                break
            global_step += 1
            learning_rate = scheduled_lr(
                global_step,
                base=args.lr,
                warmup=args.warmup_steps,
                total=planned_steps,
                minimum_ratio=args.minimum_lr_ratio,
            )
            for group in optimizer.param_groups:
                group["lr"] = learning_rate
            target, semantic, style = select_examples(
                batch,
                positions_per_sequence=args.positions_per_sequence,
                device=device,
                generator=generator,
            )
            optimizer.zero_grad(set_to_none=True)
            with autocast_context(device, args.precision):
                loss, metrics, _ = field_complete_writer_loss(
                    writer,
                    pvf.retina,
                    target,
                    semantic,
                    style,
                    stroke_weight=args.stroke_weight,
                    dice_weight=args.dice_weight,
                    pixel_l1_weight=args.pixel_l1_weight,
                    edge_weight=args.edge_weight,
                    identity_weight=args.identity_weight,
                    contrastive_weight=args.contrastive_weight,
                    field_margin_weight=args.field_margin_weight,
                    field_margin=args.field_margin,
                    zero_margin_weight=args.zero_margin_weight,
                    zero_margin=args.zero_margin,
                    coarse_weight=args.coarse_weight,
                    duplicate_similarity=args.duplicate_similarity,
                    logit_scale=args.logit_scale,
                )
            scaler.scale(loss).backward()
            scaler.unscale_(optimizer)
            gradient_norm = torch.nn.utils.clip_grad_norm_(
                trainable,
                args.gradient_clip,
            )
            scaler.step(optimizer)
            scaler.update()
            examples = target.shape[0]
            for key, value in {"loss": loss.detach(), **metrics}.items():
                running[key] = running.get(key, 0.0) + float(value) * examples
            running_examples += examples

            if global_step % args.log_every == 0:
                now = time.perf_counter()
                report = {
                    "stage": "train",
                    "route_mode": args.route_mode,
                    "step": global_step,
                    "epoch": epoch,
                    "learning_rate": learning_rate,
                    "gradient_norm": float(gradient_norm),
                    "examples_per_second": running_examples
                    / max(1e-6, now - interval_started),
                    **{
                        key: value / max(1, running_examples)
                        for key, value in running.items()
                    },
                }
                if device.type == "cuda":
                    report["peak_cuda_gib"] = (
                        torch.cuda.max_memory_allocated(device) / 2**30
                    )
                print(json.dumps(report), flush=True)
                append_jsonl(log_path, report)
                running.clear()
                running_examples = 0
                interval_started = now

            if global_step % args.validate_every == 0:
                validation = validate(
                    pvf,
                    writer,
                    development_loader,
                    device=device,
                    args=args,
                    step=global_step,
                    sample_root=output / "development_samples",
                )
                report = {
                    "stage": "validation",
                    "route_mode": args.route_mode,
                    "step": global_step,
                    **validation,
                }
                print(json.dumps(report), flush=True)
                append_jsonl(log_path, report)
                if selection_eligible(validation, args.route_mode):
                    candidate = development_snapshot(
                        validation,
                        global_step,
                        args.route_mode,
                    )
                    if better_development_candidate(candidate, best_development):
                        best_development = candidate
                        elapsed = elapsed_before + time.perf_counter() - started
                        payload = checkpoint_payload(
                            writer,
                            optimizer,
                            args=args,
                            pvf_checkpoint=pvf_checkpoint,
                            pvf_sha256=pvf_sha256,
                            partition=partition,
                            best_development=best_development,
                            epoch=epoch,
                            step=global_step,
                            elapsed_seconds=elapsed,
                        )
                        atomic_save(
                            payload,
                            output / "checkpoint_selected_development.pt",
                        )
                        _write_selection_receipt(
                            output,
                            args.route_mode,
                            best_development,
                        )

            if global_step % args.save_every == 0:
                elapsed = elapsed_before + time.perf_counter() - started
                payload = checkpoint_payload(
                    writer,
                    optimizer,
                    args=args,
                    pvf_checkpoint=pvf_checkpoint,
                    pvf_sha256=pvf_sha256,
                    partition=partition,
                    best_development=best_development,
                    epoch=epoch,
                    step=global_step,
                    elapsed_seconds=elapsed,
                )
                atomic_save(payload, output / f"checkpoint_step_{global_step:07d}.pt")
                atomic_save(payload, output / "checkpoint_latest.pt")
        if stop_requested or global_step >= planned_steps:
            break

    elapsed = elapsed_before + time.perf_counter() - started
    payload = checkpoint_payload(
        writer,
        optimizer,
        args=args,
        pvf_checkpoint=pvf_checkpoint,
        pvf_sha256=pvf_sha256,
        partition=partition,
        best_development=best_development,
        epoch=epoch,
        step=global_step,
        elapsed_seconds=elapsed,
    )
    atomic_save(payload, output / "checkpoint_latest.pt")
    final = {
        "stage": "stopped" if stop_requested else "complete",
        "route_mode": args.route_mode,
        "step": global_step,
        "elapsed_seconds": elapsed,
        "best_development": best_development,
        "checkpoint": str(output / "checkpoint_latest.pt"),
        "smoke_only": args.smoke,
        "paired_control_gate_passed": False,
        "blinded_readability_gate_passed": False,
        "frozen_evaluation_permitted": False,
    }
    print(json.dumps(final), flush=True)
    append_jsonl(log_path, final)


if __name__ == "__main__":
    main()
